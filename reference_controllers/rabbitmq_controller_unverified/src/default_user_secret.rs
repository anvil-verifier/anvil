...
pub fn default_user_secret_build(rabbitmq: &RabbitmqCluster) -> corev1::Secret {
    ...
    corev1::Secret {
        ...
        (   "host".to_string(),
            ByteString(format!("{}.{}.svc", rabbitmq.metadata.name.as_ref().unwrap(), rabbitmq.metadata.namespace.as_ref().unwrap()).into_bytes()),
        ),
        (   "provider".to_string(),
            ByteString("rabbitmq".to_string().into_bytes()),
        ),  
        (   "default_user.conf".to_string(),
            ByteString("default_user = zichengma\ndefault_pass = changeme".to_string().into_bytes()), // not sure how to do this
        ),      
        (   "port".to_string(),
