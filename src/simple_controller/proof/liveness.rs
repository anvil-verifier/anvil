        ...
        ),
{
    ...
    let added_event_in_flight = |s: State<SimpleReconcileState>| s.message_in_flight(form_msg(HostId::KubernetesAPI, HostId::CustomController, added_event(cr)));
    ...
    assert(sm_spec(simple_reconciler()).entails(
        lift_state(|s: State<SimpleReconcileState>| {
            ||| s.message_in_flight(form_msg(HostId::KubernetesAPI, HostId::CustomController, added_event(cr)))
            ||| s.reconcile_state_contains(cr.key)
            ||| s.reconcile_scheduled_for(cr.key)
        }).leads_to(always(lift_state(|s: State<SimpleReconcileState>| s.resource_key_exists(simple_reconciler::subresource_configmap(cr.key).key))))
    ));
...
proof fn lemma_cr_added_event_msg_sent_leads_to_cm_always_exists(cr: ResourceObj)
    ...
    ensures
        sm_spec(simple_reconciler()).entails(
            lift_state(|s: State<SimpleReconcileState>| s.message_in_flight(form_msg(HostId::KubernetesAPI, HostId::CustomController, added_event(cr))))
                .leads_to(always(lift_state(cm_exists(cr.key))))
        ),
{
    let cr_added_event_msg_sent = |s: State<SimpleReconcileState>| s.message_in_flight(form_msg(HostId::KubernetesAPI, HostId::CustomController, added_event(cr)));
    let reconcile_ongoing = |s: State<SimpleReconcileState>| s.reconcile_state_contains(cr.key);
        ...
        ),
{
    ...
    let cr_added_event_msg = form_msg(HostId::KubernetesAPI, HostId::CustomController, added_event(cr));
