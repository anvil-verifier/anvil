        ...
        sm_spec().entails(lift_state(message_sent(msg)).leads_to(lift_state(post))),
{
    ...
    // F:
    assert(forall |s, s_prime: State| kubernetes_api_next().pre(recv)(s) && #[trigger] next()(s, s_prime) ==> kubernetes_api_next().pre(recv)(s_prime) || post(s_prime));
    assert(forall |s, s_prime: State| kubernetes_api_next().pre(recv)(s) && #[trigger] next()(s, s_prime) && kubernetes_api_next().forward(recv)(s, s_prime) ==> post(s_prime));
