        ...
        }))))),
{
    ...
    use_tla_forall::<State, Option<Message>>(sm_spec(), |recv| weak_fairness(distributed_system::controller_next().pred(recv)), Option::Some(msg));

    distributed_system::controller_action_enabled(Option::Some(msg), controller::send_create_sts());

    wf1::<State>(sm_spec(),
        next(),
        distributed_system::controller_next().pred(Option::Some(msg)),
