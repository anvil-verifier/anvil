...
pub proof fn lemma_from_receive_list_resp_and_no_updated_vrs_exists_to_send_create_vrs_req(
    vd: VDeploymentView, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int
)
requires
    cluster.type_is_installed_in_cluster::<VDeploymentView>(),
    ...
    spec.entails(tla_forall(|i: (Option<Message>, Option<ObjectRef>)| cluster.controller_next().weak_fairness((controller_id, i.0, i.1)))),
ensures
    spec.entails(lift_state(and!(at_vd_step_with_vd(vd, controller_id, at_step_or!(AfterListVRS)), exists_list_resp_in_flight(vd, controller_id), etcd_new_vrs_does_not_exist(vd)))
       .leads_to(lift_state(and!(at_vd_step_with_vd(vd, controller_id, at_step_or!(AfterCreateVRS)), pending_create_req_in_flight(vd, controller_id))))),
    {}
