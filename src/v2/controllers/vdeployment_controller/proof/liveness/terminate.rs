    ...
    )))),
{
    ...
    assert forall |ex| #[trigger] spec.satisfied_by(ex) && spec.entails(always(pre)) implies always(post).satisfied_by(ex) by {
        ...
        assert(forall |i: nat| #[trigger] pre.satisfied_by(ex.suffix(i)) ==> post.satisfied_by(ex.suffix(i)));
    }
}
}