    ...
    assert_by(
        lhs ==> rhs,
        {
            assert forall |ex: Execution<ClusterState>, n: nat, other_id: int| #![auto]
                ...
                implies vrs_not_interfered_by(controller_id, other_id)(ex.suffix(n).head()) by {
                // Gradually unwrap the semantics of `spec.entails(always(lift_state(vrs_not_interfered_by(controller_id, other_id))))`
                // until Verus can show the consequent.
                assert(valid(spec.implies(always(lift_state(vrs_not_interfered_by(controller_id, other_id))))));
                assert(spec.implies(always(lift_state(vrs_not_interfered_by(controller_id, other_id)))).satisfied_by(ex));
                assert(always(lift_state(vrs_not_interfered_by(controller_id, other_id))).satisfied_by(ex));
                assert(lift_state(vrs_not_interfered_by(controller_id, other_id)).satisfied_by(ex.suffix(n)));
            }
        }
    );
    ...
    assert_by(
        lhs ==> rhs,
        {
            assert forall |ex: Execution<ClusterState>, n: nat, other_id: int| #![auto]
                ...
                && cluster.controller_models.remove(controller_id).contains_key(other_id)
                implies 
                vrs_not_interfered_by(controller_id, other_id)(ex.suffix(n).head())
                && vrs_not_interfered_by(controller_id, other_id)(ex.suffix(n).head_next()) by {
                // Gradually unwrap the semantics of `spec.entails(always(lift_state(vrs_not_interfered_by(controller_id, other_id))))`
                // until Verus can show the consequent.
                assert(valid(spec.implies(always(lift_state(vrs_not_interfered_by(controller_id, other_id))))));
                assert(spec.implies(always(lift_state(vrs_not_interfered_by(controller_id, other_id)))).satisfied_by(ex));
                assert(always(lift_state(vrs_not_interfered_by(controller_id, other_id))).satisfied_by(ex));
                assert(lift_state(vrs_not_interfered_by(controller_id, other_id)).satisfied_by(ex.suffix(n)));
                assert(lift_state(vrs_not_interfered_by(controller_id, other_id)).satisfied_by(ex.suffix(n + 1)));
            }
        }
    );
