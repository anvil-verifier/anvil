...
use vstd::{imap_lib::*, prelude::*, seq_lib::*, iset::*, iset_lib::*};

verus! {

pub proof fn a_submap_of_a_finite_map_is_finite<K, V>(m1: IMap<K, V>, m2: IMap<K, V>)
    requires
        m1.submap_of(m2),
        m2.dom().finite(),
    ensures
...
pub proof fn injective_finite_map_implies_dom_len_is_equal_to_values_len<K, V>(m: IMap<K, V>)
    requires
        m.dom().finite(),
        m.is_injective(),
    ensures
        ...
        m.dom().len(),
{
    if m.dom().len() == 0 {
        ...
        assert(m.values() =~= ISet::empty());
    } else {
        let key = m.dom().choose();
        let value = m[key];
        let singleton_value = ISet::empty().insert(value);
        ...
        lemma_iset_disjoint_lens(submap.values(), singleton_value);
        assert(m.values().len() == submap.values().len() + 1);
    }
}

pub proof fn lemma_equiv_filters_on_keys_and_values_implies_equiv_results<K, V>(m: IMap<K, V>, f: spec_fn(K) -> bool, g: spec_fn(V) -> bool, v_k_map: spec_fn(V) -> K)
requires
    m.is_injective(),
