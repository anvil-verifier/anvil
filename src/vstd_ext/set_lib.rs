...
pub proof fn lemma_set_to_seq_contains_all_elements<A>(s: Set<A>)
    ...
    decreases s.len()
{
    ...
    if s.len() != 0 {
        let x = s.choose();
        lemma_set_to_seq_has_no_duplicates(s.remove(x));
        lemma_set_to_seq_contains_all_elements(s.remove(x));
        assert(!s.remove(x).to_seq().contains(x));
    }
}

proof fn element_in_finite_set_exists_in_set_to_seq<A>(s: Set<A>, e: A)
    ensures s.contains(e) <==> s.to_seq().contains(e),
    decreases s.len()
{
    if s.len() != 0 {
