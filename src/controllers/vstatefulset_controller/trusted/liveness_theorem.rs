    ...
    |s: ClusterState| {
        ...
        &&& forall |ord: nat| #![trigger pod_name(vsts.metadata.name->0, ord)] 0 <= ord < vsts.spec.replicas.unwrap_or(1) ==> {
            ...
            &&& forall |i: int| #![trigger vsts.spec.volume_claim_templates->0[i]] 0 <= i < pvc_template_cnt ==> {
                ...
                &&& exists |obj| {
                    &&& s.resources().contains_value(obj)
                    &&& #[trigger] bound_dyn_pvc_filter(pvc_template_name, vsts, ord)(obj)
                }
            }
        }
...
// without naming requirements, because itself alone is already guarded by rely-conditions
