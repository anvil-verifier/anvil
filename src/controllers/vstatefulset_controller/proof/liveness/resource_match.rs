    ...
    pvc_needed_condemned_index_condemned_len_and_outdated_len_are(vsts, controller_id, pvc_cnt(vsts), needed_index + nat1!(), nat0!(), condemned_len, outdated_len)(s_prime),
{
    ...
    assert(req_obj.metadata == make_pod(vsts, needed_index).metadata);
    assert(req_pod.metadata.owner_references == Some(seq![vsts.controller_owner_ref()]));
    assert(req_obj.metadata.namespace == Some(vsts.metadata.namespace->0));
}
