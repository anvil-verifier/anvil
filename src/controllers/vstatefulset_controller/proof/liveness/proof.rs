...
pub proof fn spec_of_previous_phases_entails_eventually_new_invariants(provided_spec: TempPred<ClusterState>, vsts: VStatefulSetView, cluster: Cluster, controller_id: int, i: nat)
    requires
        ...
        provided_spec.entails(always(lift_state(vsts_rely_conditions_pod_monkey()))),
    ensures
...
pub open spec fn step_sends_request(step: VStatefulSetReconcileStepView) -> bool {
    ...
    ||| step is AfterGetPVC
    ||| step is AfterCreatePVC
    ||| step is AfterCreateNeeded
    ||| step is AfterUpdateNeeded
    ||| step is AfterDeleteCondemned
    ||| step is AfterDeleteOutdated
}

proof fn reconcile_core_request_implies_after_step(vsts: VStatefulSetView, resp_o: Option<ResponseView<VoidERespView>>, state: VStatefulSetReconcileState)
    ensures
...
proof fn no_pending_req_msg_at_non_after_steps(cluster: Cluster, controller_id: int)
    requires
        cluster.type_is_installed_in_cluster::<VStatefulSetView>(),
        cluster.controller_models.contains_pair(controller_id, vsts_controller_model()),
    ensures
            ...
            ==> (cluster.controller_models[controller_id].reconcile_model.transition)(cr, resp_o, pre_state).1 is None,
{
    ...
    assert forall |cr: DynamicObjectView, resp_o: Option<ResponseContent>, pre_state: ReconcileLocalState|
        ...
        reconcile_core_request_implies_after_step(
            VStatefulSetView::unmarshal(cr)->Ok_0, resp_o_um, VStatefulSetReconcileState::unmarshal(pre_state)->Ok_0);
    }
}
        ...
        spec.entails(always(lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, at_step_or![Init])))),
{
    ...
    no_pending_req_msg_at_non_after_steps(cluster, controller_id);
        ...
        spec.entails(always(tla_forall(|vsts: VStatefulSetView| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, vsts.object_ref(), cluster.reconcile_model(controller_id).done))))),
{
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), cluster.reconcile_model(controller_id).done)))) by {
        spec_entails_no_pending_req_msg_at_done_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
...
#[verifier(rlimit(200))]
#[verifier(spinoff_prover)]
proof fn spec_entails_no_pending_req_msg_at_get_pvc_for_key(spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, key: ObjectRef)
    requires
        spec.entails(lift_state(cluster.init())),
        spec.entails(always(lift_action(cluster.next()))),
        cluster.type_is_installed_in_cluster::<VStatefulSetView>(),
        ...
        spec.entails(always(tla_forall(|vsts: VStatefulSetView| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, vsts.object_ref(), at_step_or![GetPVC]))))),
{
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), at_step_or![GetPVC])))) by {
        spec_entails_no_pending_req_msg_at_get_pvc_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
...
{
    spec_entails_pending_request_invariants_part4_create_needed(spec, cluster, controller_id);
...
#[verifier(rlimit(200))]
#[verifier(spinoff_prover)]
proof fn spec_entails_no_pending_req_msg_at_update_needed_for_key(spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, key: ObjectRef)
    requires
        spec.entails(lift_state(cluster.init())),
        spec.entails(always(lift_action(cluster.next()))),
        cluster.type_is_installed_in_cluster::<VStatefulSetView>(),
        ...
        spec.entails(always(tla_forall(|vsts: VStatefulSetView| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, vsts.object_ref(), at_step_or![UpdateNeeded]))))),
{
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), at_step_or![UpdateNeeded])))) by {
        spec_entails_no_pending_req_msg_at_update_needed_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
...
#[verifier(rlimit(200))]
#[verifier(spinoff_prover)]
proof fn spec_entails_no_pending_req_msg_at_delete_condemned_for_key(spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, key: ObjectRef)
    requires
        spec.entails(lift_state(cluster.init())),
        spec.entails(always(lift_action(cluster.next()))),
        cluster.type_is_installed_in_cluster::<VStatefulSetView>(),
        ...
        spec.entails(always(tla_forall(|vsts: VStatefulSetView| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, vsts.object_ref(), at_step_or![DeleteOutdated]))))),
{
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), at_step_or![DeleteCondemned])))) by {
        spec_entails_no_pending_req_msg_at_delete_condemned_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
    ...
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), at_step_or![DeleteOutdated])))) by {
        spec_entails_no_pending_req_msg_at_delete_outdated_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
        ...
        spec.entails(always(tla_forall(|vsts: VStatefulSetView| lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(controller_id, vsts.object_ref(), at_step_or![AfterCreatePVC]))))),
{
    ...
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), at_step_or![AfterGetPVC])))) by {
        spec_entails_pending_req_or_resp_in_flight_at_aftergetpvc_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
    ...
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), at_step_or![AfterCreatePVC])))) by {
        spec_entails_pending_req_or_resp_in_flight_at_aftercreatepvc_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
        ...
        spec.entails(always(tla_forall(|vsts: VStatefulSetView| lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(controller_id, vsts.object_ref(), at_step_or![AfterUpdateNeeded]))))),
{
    ...
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), at_step_or![AfterUpdateNeeded])))) by {
        spec_entails_pending_req_or_resp_in_flight_at_afterupdateneeded_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
        ...
        spec.entails(always(tla_forall(|vsts: VStatefulSetView| lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(controller_id, vsts.object_ref(), at_step_or![AfterDeleteOutdated]))))),
{
    ...
    assert forall |vsts: VStatefulSetView| spec.entails(always(lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(controller_id, #[trigger] vsts.object_ref(), at_step_or![AfterDeleteOutdated])))) by {
        spec_entails_pending_req_or_resp_in_flight_at_afterdeleteoutdated_for_key(spec, cluster, controller_id, vsts.object_ref());
    }
        ...
        spec.entails(derived_invariants_since_beginning(vsts, cluster, controller_id)),
{
    ...
    helper_invariants::lemma_always_all_pvcs_in_etcd_matching_vsts_have_no_finalizer_or_deletion_timestamp_or_owner_ref(spec, cluster, controller_id);
    helper_invariants::lemma_always_buildin_controllers_do_not_delete_pvcs_owned_by_vsts(spec, cluster, controller_id);
    helper_invariants::lemma_always_every_msg_from_vsts_controller_carries_vsts_key(spec, cluster, controller_id);
    cluster.lemma_always_every_in_flight_msg_from_controller_has_kind_as::<VStatefulSetView>(spec, controller_id);
    helper_invariants::lemma_always_all_pods_in_etcd_matching_vsts_have_no_finalizer_or_deletion_timestamp_and_one_owner_ref(spec, cluster, controller_id, vsts);
...
ensures
    assumption_and_invariants_of_all_phases(vsts, cluster, controller_id)
        ...
        .entails(always(lift_state(#[trigger] cluster_invariants_since_reconciliation(cluster, vsts, controller_id)))),
{
    let stable_spec = assumption_and_invariants_of_all_phases(vsts, cluster, controller_id)
        .and(always(lift_state(vsts_rely_conditions(cluster, controller_id))));

    // Extract from invariants_since_phase_i
    ...
    // Extract from derived_invariants_since_beginning
    ...
    // Extract pending_req_of_key for specific vsts from the forall version
    entails_trans(stable_spec,
        derived_invariants_since_beginning(vsts, cluster, controller_id),
    ...
    always_tla_forall_apply(stable_spec, |vsts: VStatefulSetView| lift_state(Cluster::pending_req_of_key_is_unique_with_unique_id(controller_id, vsts.object_ref())), vsts);
    ...
    // Extract from assumption_and_invariants_of_all_phases
    assert(stable_spec.entails(always(lift_state(Cluster::desired_state_is(vsts)))));

    // Extract from invariants_since_phase_iii
    assert(stable_spec.entails(always(lift_state(Cluster::every_msg_from_key_is_pending_req_msg_of(controller_id, vsts.object_ref())))));

    // Extract rely conditions
    assert(stable_spec.entails(always(lift_state(vsts_rely_conditions(cluster, controller_id)))));

    // Extract from invariants_since_phase_i
    assert(stable_spec.entails(always(lift_state(Cluster::the_object_in_schedule_has_spec_and_uid_as(controller_id, vsts)))));
    ...
    // These are semantically identical (both check that controller_id messages have VStatefulSetView kind)
    // Bridge: helper_invariants version implies internal_rely_guarantee version
    // Then use always_weaken to get always(internal_rely_guarantee version) from always(helper_invariants version)
    assert(lift_state(helper_invariants::every_msg_from_vsts_controller_carries_vsts_key(controller_id)).entails(
        lift_state(internal_rely_guarantee::every_msg_from_vsts_controller_carries_vsts_key(controller_id))
    )) by {
        assert forall |ex: Execution<ClusterState>|
            lift_state(helper_invariants::every_msg_from_vsts_controller_carries_vsts_key(controller_id)).satisfied_by(ex)
        implies #[trigger]
            lift_state(internal_rely_guarantee::every_msg_from_vsts_controller_carries_vsts_key(controller_id)).satisfied_by(ex)
        by {
            let s = ex.head();
            assert forall |msg: Message| #[trigger] s.in_flight().contains(msg)
                && msg.src.is_controller_id(controller_id)
            implies msg.src->Controller_1.kind == VStatefulSetView::kind() by {
                assert(msg.src is Controller && msg.src->Controller_0 == controller_id);
            }
        }
    }
    always_weaken(stable_spec,
        lift_state(helper_invariants::every_msg_from_vsts_controller_carries_vsts_key(controller_id)),
        lift_state(internal_rely_guarantee::every_msg_from_vsts_controller_carries_vsts_key(controller_id))
    );
