...
// The names generated by the vstatefulset controller are all built by concatenating
// the "vstatefulset" prefix, dashes, the (ascii) parent name / pvc template name and
// the decimal representation of an ordinal. All the pieces are ascii, so the result is.
...
// The vsts object lives in etcd (by desired_state_is), and every object in etcd has
// well-formed metadata, which requires the name to be ascii.
