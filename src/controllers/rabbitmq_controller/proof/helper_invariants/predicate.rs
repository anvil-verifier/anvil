...
pub open spec fn the_object_in_reconcile_satisfies_state_validation(key: ObjectRef) -> StatePred<ClusterState>
{
    |s: ClusterState| {
...
pub open spec fn the_object_in_schedule_satisfies_state_validation() -> StatePred<ClusterState>
{
    |s: ClusterState| {
...
pub open spec fn every_resource_update_request_implies_at_after_update_resource_step(sub_resource: SubResource, rabbitmq: RabbitmqClusterView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        let key = rabbitmq.object_ref();
        let resource_key = get_request(sub_resource, rabbitmq).key;
        forall |msg: RMQMessage| {
...
pub open spec fn cm_rv_is_the_same_as_etcd_server_cm_if_cm_updated(rabbitmq: RabbitmqClusterView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        let key = rabbitmq.object_ref();
...
pub open spec fn cm_rv_stays_unchanged(rabbitmq: RabbitmqClusterView) -> ActionPred<Cluster> {
    |s: ClusterState, s_prime: Cluster| {
        let cm_key = get_request(SubResource::ServerConfigMap, rabbitmq).key;
        &&& s.resources().contains_key(cm_key)
