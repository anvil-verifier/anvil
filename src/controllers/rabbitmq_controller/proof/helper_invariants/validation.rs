        ...
        implies s_prime.resources()[sts_key].metadata.owner_references == msg.content.get_update_request().obj.metadata.owner_references by {
            ...
            match step {
                Step::APIServerStep(input) => {
                    if s_prime.resources()[sts_key] != s.resources()[sts_key] {
                        assert(s_prime.resources()[sts_key].metadata.resource_version->0 != s.resources()[sts_key].metadata.resource_version->0);
                    }
                },
