...
pub open spec fn spec_before_phase_n(n: nat, rabbitmq: RabbitmqClusterView) -> TempPred<ClusterState>
    decreases n,
{
    if n == 1 {
        invariants(rabbitmq).and(always(lift_state(desired_state_is(rabbitmq))))
    ...
    ensures spec_before_phase_n(i, rabbitmq).entails(true_pred().leads_to(invariants_since_phase_n(i, rabbitmq))),
{
    ...
    if i == 1 {
        Cluster::lemma_true_leads_to_crash_always_disabled(spec);
        Cluster::lemma_true_leads_to_busy_always_disabled(spec);
        Cluster::lemma_true_leads_to_always_the_object_in_schedule_has_spec_and_uid_as(spec, rabbitmq);
        ...
        if i == 2 {
            Cluster::lemma_true_leads_to_always_the_object_in_reconcile_has_spec_and_uid_as(spec, rabbitmq);
        } else if i == 3 {
...
pub open spec fn next_with_wf() -> TempPred<ClusterState> {
    ...
    .and(Cluster::disable_crash().weak_fairness(()))
    ...
    ensures valid(stable(next_with_wf())),
{
    ...
    stable_and_n!(
        ...
        Cluster::disable_crash().weak_fairness(()),
        Cluster::disable_transient_failure().weak_fairness(())
    );
}
...
pub open spec fn derived_invariants_since_beginning(rabbitmq: RabbitmqClusterView) -> TempPred<ClusterState> {
    ...
    .and(always(lift_state(Cluster::no_pending_req_msg_at_reconcile_state(rabbitmq.object_ref(), at_step_closure(RabbitmqReconcileStep::Init)))))
    .and(always(tla_forall(|step: (ActionKind, SubResource)| lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(rabbitmq.object_ref(), at_step_closure(RabbitmqReconcileStep::AfterKRequestStep(step.0, step.1)))))))
    ...
    .and(always(lift_state(Cluster::key_of_object_in_matched_ok_get_resp_message_is_same_as_key_of_pending_req(rabbitmq.object_ref()))))
    .and(always(lift_state(Cluster::key_of_object_in_matched_ok_create_resp_message_is_same_as_key_of_pending_req(rabbitmq.object_ref()))))
    .and(always(lift_state(Cluster::key_of_object_in_matched_ok_update_resp_message_is_same_as_key_of_pending_req(rabbitmq.object_ref()))))
    ...
    ensures valid(stable(derived_invariants_since_beginning(rabbitmq))),
{
    ...
    stable_and_always_n!(
        ...
        tla_forall(a_to_p_1),
        lift_state(Cluster::no_pending_req_msg_at_reconcile_state(rabbitmq.object_ref(), at_step_closure(RabbitmqReconcileStep::Init))),
        tla_forall(a_to_p_2),
        tla_forall(a_to_p_3),
        ...
        lift_state(Cluster::key_of_object_in_matched_ok_get_resp_message_is_same_as_key_of_pending_req(rabbitmq.object_ref())),
        lift_state(Cluster::key_of_object_in_matched_ok_create_resp_message_is_same_as_key_of_pending_req(rabbitmq.object_ref())),
        lift_state(Cluster::key_of_object_in_matched_ok_update_resp_message_is_same_as_key_of_pending_req(rabbitmq.object_ref())),
        tla_forall(a_to_p_4),
        tla_forall(a_to_p_5),
...
pub open spec fn invariants_since_phase_i(rabbitmq: RabbitmqClusterView) -> TempPred<ClusterState> {
    always(lift_state(Cluster::crash_disabled()))
    .and(always(lift_state(Cluster::busy_disabled())))
    .and(always(lift_state(Cluster::the_object_in_schedule_has_spec_and_uid_as(rabbitmq))))
}
...
pub open spec fn invariants_since_phase_ii(rabbitmq: RabbitmqClusterView) -> TempPred<ClusterState> {
    always(lift_state(Cluster::the_object_in_reconcile_has_spec_and_uid_as(rabbitmq)))
}
...
{
    always_p_is_stable(lift_state(Cluster::the_object_in_reconcile_has_spec_and_uid_as(rabbitmq)));
}

pub open spec fn invariants_since_phase_iii(rabbitmq: RabbitmqClusterView) -> TempPred<ClusterState> {
...
pub open spec fn invariants_since_phase_vi(rabbitmq: RabbitmqClusterView) -> TempPred<ClusterState> {
...
pub open spec fn invariants_since_phase_vii(rabbitmq: RabbitmqClusterView) -> TempPred<ClusterState> {
        ...
        spec.entails(always(tla_forall(|step: (ActionKind, SubResource)| lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(rabbitmq.object_ref(), at_step_closure(RabbitmqReconcileStep::AfterKRequestStep(step.0, step.1))))))),
{
    ...
    let a_to_p = |step: (ActionKind, SubResource)| lift_state(Cluster::pending_req_in_flight_or_resp_in_flight_at_reconcile_state(rabbitmq.object_ref(), at_step_closure(RabbitmqReconcileStep::AfterKRequestStep(step.0, step.1))));
    ...
    ensures cluster_spec().entails(derived_invariants_since_beginning(rabbitmq)),
{
    ...
    Cluster::lemma_always_pending_req_of_key_is_unique_with_unique_id(spec, rabbitmq.object_ref());
    ...
    Cluster::lemma_always_each_scheduled_object_has_consistent_key_and_valid_metadata(spec);
    ...
    Cluster::lemma_always_no_pending_req_msg_at_reconcile_state(spec, rabbitmq.object_ref(), at_step_closure(RabbitmqReconcileStep::Init));
    ...
    Cluster::lemma_always_key_of_object_in_matched_ok_get_resp_message_is_same_as_key_of_pending_req(spec, rabbitmq.object_ref());
    Cluster::lemma_always_key_of_object_in_matched_ok_create_resp_message_is_same_as_key_of_pending_req(spec, rabbitmq.object_ref());
    Cluster::lemma_always_key_of_object_in_matched_ok_update_resp_message_is_same_as_key_of_pending_req(spec, rabbitmq.object_ref());
    let a_to_p_4 = |res: SubResource| lift_state(helper_invariants::response_at_after_get_resource_step_is_resource_get_response(res, rabbitmq));
