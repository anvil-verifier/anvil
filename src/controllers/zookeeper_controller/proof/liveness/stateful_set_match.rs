...
pub proof fn lemma_from_after_get_stateful_set_step_to_stateful_set_matches(
    ...
    requires
        ...
        spec.entails(always(lift_state(helper_invariants::every_resource_update_request_implies_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::stateful_set_not_exists_or_matches_or_no_more_status_update(zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::no_delete_resource_request_msg_in_flight(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::no_create_resource_request_msg_without_name_in_flight(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::cm_rv_is_the_same_as_etcd_server_cm_if_cm_updated(zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::resource_object_only_has_owner_reference_pointing_to_current_cr(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::object_in_etcd_satisfies_unchangeable(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::resource_object_has_no_finalizers_or_timestamp_and_only_has_controller_owner_ref(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::every_resource_create_request_implies_at_after_create_resource_step(SubResource::VStatefulSetView, zookeeper)))),
    ...
    ensures spec.entails(lift_state(pending_req_in_flight_at_after_get_resource_step(SubResource::VStatefulSetView, zookeeper)).leads_to(lift_state(sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)))),
{
    lemma_from_after_get_resource_step_and_key_not_exists_to_resource_matches(spec, SubResource::VStatefulSetView, zookeeper);
    ...
    let key_not_exists = lift_state(|s: ZKCluster| {
        &&& !s.resources().contains_key(get_request(SubResource::VStatefulSetView, zookeeper).key)
        &&& pending_req_in_flight_at_after_get_resource_step(SubResource::VStatefulSetView, zookeeper)(s)
    });
    let key_exists = lift_state(|s: ZKCluster| {
        &&& s.resources().contains_key(get_request(SubResource::VStatefulSetView, zookeeper).key)
        &&& pending_req_in_flight_at_after_get_resource_step(SubResource::VStatefulSetView, zookeeper)(s)
    });
    or_leads_to_combine(spec, key_not_exists, key_exists, lift_state(sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)));
    temp_pred_equality(
        key_not_exists.or(key_exists), lift_state(pending_req_in_flight_at_after_get_resource_step(SubResource::VStatefulSetView, zookeeper))
    );
}
    ...
    ensures
        spec.entails(
            lift_state(|s: ZKCluster| {
                &&& s.resources().contains_key(get_request(SubResource::VStatefulSetView, zookeeper).key)
                &&& pending_req_in_flight_at_after_get_resource_step(SubResource::VStatefulSetView, zookeeper)(s)
            }).leads_to(lift_state(sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)))
        ),
{
    ...
    let post = lift_state(sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper));
    ...
    assert_by(spec.entails(stateful_set_not_matches.leads_to(post)), {
        let pre1 = |req_msg| lift_state(|s: ZKCluster| {
            &&& req_msg_is_the_in_flight_pending_req_at_after_get_resource_step_and_key_exists(SubResource::VStatefulSetView, zookeeper, req_msg)(s)
            &&& !sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)(s)
        });
        let post1 = lift_state(|s: ZKCluster| {
            &&& at_after_get_resource_step_and_exists_ok_resp_in_flight(SubResource::VStatefulSetView, zookeeper)(s)
            &&& !sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)(s)
        });
        ...
        let pre2 = |resp_msg| lift_state(|s: ZKCluster| {
            &&& resp_msg_is_the_in_flight_ok_resp_at_after_get_resource_step(SubResource::VStatefulSetView, zookeeper, resp_msg)(s)
            &&& !sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)(s)
        });
        let post2 = lift_state(|s: ZKCluster| {
            &&& pending_req_in_flight_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper)(s)
            &&& !sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)(s)
        });
        ...
        let pre3 = |req_msg| lift_state(|s: ZKCluster| {
            &&& req_msg_is_the_in_flight_pending_req_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper, req_msg)(s)
            &&& !sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)(s)
        });
        ...
        ),
{
    ...
    let resource_key = get_request(SubResource::VStatefulSetView, zookeeper).key;
    ...
    let stronger_next = |s, s_prime: ZKCluster| {
        ...
        &&& helper_invariants::every_resource_update_request_implies_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper)(s)
        &&& helper_invariants::no_delete_resource_request_msg_in_flight(SubResource::VStatefulSetView, zookeeper)(s)
    ...
    combine_spec_entails_always_n!(
        ...
        lift_state(helper_invariants::every_resource_update_request_implies_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper)),
        lift_state(helper_invariants::no_delete_resource_request_msg_in_flight(SubResource::VStatefulSetView, zookeeper)),
        ...
        ),
{
    ...
    let stronger_next = |s, s_prime: ZKCluster| {
        ...
        &&& helper_invariants::object_in_etcd_satisfies_unchangeable(SubResource::VStatefulSetView, zookeeper)(s)
        &&& helper_invariants::resource_object_only_has_owner_reference_pointing_to_current_cr(SubResource::VStatefulSetView, zookeeper)(s)
    ...
    combine_spec_entails_always_n!(
        ...
        lift_state(helper_invariants::object_in_etcd_satisfies_unchangeable(SubResource::VStatefulSetView, zookeeper)),
        lift_state(helper_invariants::resource_object_only_has_owner_reference_pointing_to_current_cr(SubResource::VStatefulSetView, zookeeper)),
        ...
        ),
{
    ...
    let post = sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper);
    ...
    let stronger_next = |s, s_prime: ZKCluster| {
        ...
        &&& helper_invariants::no_delete_resource_request_msg_in_flight(SubResource::VStatefulSetView, zookeeper)(s)
        &&& helper_invariants::stateful_set_not_exists_or_matches_or_no_more_status_update(zookeeper)(s)
        &&& helper_invariants::resource_object_has_no_finalizers_or_timestamp_and_only_has_controller_owner_ref(SubResource::VStatefulSetView, zookeeper)(s)
        &&& helper_invariants::object_in_etcd_satisfies_unchangeable(SubResource::VStatefulSetView, zookeeper)(s)
        &&& helper_invariants::cm_rv_is_the_same_as_etcd_server_cm_if_cm_updated(zookeeper)(s)
        &&& helper_invariants::resource_object_only_has_owner_reference_pointing_to_current_cr(SubResource::VStatefulSetView, zookeeper)(s)
        &&& helper_invariants::stateful_set_in_etcd_satisfies_unchangeable(zookeeper)(s)
    ...
    combine_spec_entails_always_n!(
        ...
        lift_state(helper_invariants::no_delete_resource_request_msg_in_flight(SubResource::VStatefulSetView, zookeeper)),
        lift_state(helper_invariants::stateful_set_not_exists_or_matches_or_no_more_status_update(zookeeper)),
        lift_state(helper_invariants::resource_object_has_no_finalizers_or_timestamp_and_only_has_controller_owner_ref(SubResource::VStatefulSetView, zookeeper)),
        lift_state(helper_invariants::object_in_etcd_satisfies_unchangeable(SubResource::VStatefulSetView, zookeeper)),
        lift_state(helper_invariants::cm_rv_is_the_same_as_etcd_server_cm_if_cm_updated(zookeeper)),
        lift_state(helper_invariants::resource_object_only_has_owner_reference_pointing_to_current_cr(SubResource::VStatefulSetView, zookeeper)),
        lift_state(helper_invariants::stateful_set_in_etcd_satisfies_unchangeable(zookeeper))
...
pub proof fn lemma_stateful_set_is_stable(
    ...
    requires
        spec.entails(p.leads_to(lift_state(sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_action(ZKCluster::next()))),
        spec.entails(always(lift_state(helper_invariants::no_delete_resource_request_msg_in_flight(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::every_resource_update_request_implies_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::resource_object_has_no_finalizers_or_timestamp_and_only_has_controller_owner_ref(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(helper_invariants::cm_rv_is_the_same_as_etcd_server_cm_if_cm_updated(zookeeper)))),
        spec.entails(always(lift_action(helper_invariants::cm_rv_stays_unchanged(zookeeper)))),
    ensures spec.entails(p.leads_to(always(lift_state(sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper))))),
{
    let post = sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper);
    let resource_key = get_request(SubResource::VStatefulSetView, zookeeper).key;
    let stronger_next = |s, s_prime: ZKCluster| {
