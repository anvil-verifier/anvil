...
pub proof fn lemma_eventually_always_stateful_set_not_exists_or_matches_or_no_more_status_update(spec: TempPred<ZKCluster>, zookeeper: ZookeeperClusterView)
    requires
        ...
        spec.entails(always(lift_state(ZKCluster::each_object_in_reconcile_has_consistent_key_and_valid_metadata()))),
        spec.entails(always(lift_state(ZKCluster::desired_state_is(zookeeper)))),
        spec.entails(always(lift_state(every_resource_create_request_implies_at_after_create_resource_step(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(every_resource_update_request_implies_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(object_in_etcd_satisfies_unchangeable(SubResource::VStatefulSetView, zookeeper)))),
        spec.entails(always(lift_state(stateful_set_in_etcd_satisfies_unchangeable(zookeeper)))),
        spec.entails(always(lift_state(resource_object_only_has_owner_reference_pointing_to_current_cr(SubResource::VStatefulSetView, zookeeper)))),
        ...
        spec.entails(always(lift_state(no_create_resource_request_msg_without_name_in_flight(SubResource::VStatefulSetView, zookeeper)))),
    ...
    ensures spec.entails(true_pred().leads_to(always(lift_state(stateful_set_not_exists_or_matches_or_no_more_status_update(zookeeper))))),
{
    ...
    let inv_for_create = |s: ZKCluster| {
        &&& every_resource_create_request_implies_at_after_create_resource_step(SubResource::VStatefulSetView, zookeeper)(s)
    ...
    invariant_n!(
        ...
        lift_state(every_resource_create_request_implies_at_after_create_resource_step(SubResource::VStatefulSetView, zookeeper)),
        lift_state(cm_rv_is_the_same_as_etcd_server_cm_if_cm_updated(zookeeper))
    );
    let inv_for_update = |s: ZKCluster, s_prime: ZKCluster| {
        &&& ZKCluster::each_object_in_etcd_is_well_formed()(s)
        &&& ZKCluster::desired_state_is(zookeeper)(s)
        &&& every_resource_update_request_implies_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper)(s)
        &&& stateful_set_in_etcd_satisfies_unchangeable(zookeeper)(s)
        &&& resource_object_only_has_owner_reference_pointing_to_current_cr(SubResource::VStatefulSetView, zookeeper)(s)
        &&& cm_rv_is_the_same_as_etcd_server_cm_if_cm_updated(zookeeper)(s)
    ...
    invariant_n!(
        ...
        lift_state(ZKCluster::each_object_in_etcd_is_well_formed()),
        lift_state(ZKCluster::desired_state_is(zookeeper)),
        lift_state(every_resource_update_request_implies_at_after_update_resource_step(SubResource::VStatefulSetView, zookeeper)),
        lift_state(stateful_set_in_etcd_satisfies_unchangeable(zookeeper)),
        lift_state(resource_object_only_has_owner_reference_pointing_to_current_cr(SubResource::VStatefulSetView, zookeeper)),
        lift_state(cm_rv_is_the_same_as_etcd_server_cm_if_cm_updated(zookeeper)),
    ...
    implies stateful_set_not_exists_or_matches_or_no_more_status_update(zookeeper)(s) by {
        ...
        else if sub_resource_state_matches(SubResource::VStatefulSetView, zookeeper)(s) {}
        else {
            assert forall |msg: ZKMessage| s.in_flight().contains(msg) implies !(#[trigger] resource_update_status_request_msg(get_request(SubResource::VStatefulSetView, zookeeper).key)(msg)) by {
                if update_status_msg_from_bc_for(get_request(SubResource::VStatefulSetView, zookeeper).key)(msg) {} else {}
            }
        }
    }
    combine_spec_entails_always_n!(
