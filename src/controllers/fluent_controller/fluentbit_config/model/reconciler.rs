...
pub open spec fn reconcile_core(
    ...
    let resp = resp_o->0;
    let fbc_name = fbc.metadata.name->0;
    let fbc_namespace = fbc.metadata.namespace->0;
    match step {
            ...
            match action {
                ActionKind::Get => {
                    if resp_o is Some && resp_o->0.is_KResponse() && resp_o->0.get_KResponse_0().is_GetResponse() {
                        ...
                        if get_resp is Ok {
                            // update
                            let new_obj = Builder::update(fbc, state, get_resp->Ok_0);
                            if new_obj is Ok {
                ...
                ActionKind::Create => {
                    ...
                    && create_resp is Ok {
                        let next_state = Builder::state_after_create(fbc, create_resp->Ok_0, state);
                        if next_state is Ok {
                ...
                ActionKind::Update => {
                    ...
                    && update_resp is Ok {
                        let next_state = Builder::state_after_update(fbc, update_resp->Ok_0, state);
                        if next_state is Ok {
                            let (state_prime, req) = next_state->Ok_0;
                            let req_o = if req is Some { Some(RequestView::KRequest(req->0)) } else { None };
                            (state_prime, req_o)
