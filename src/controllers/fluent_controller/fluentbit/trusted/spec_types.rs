...
impl ResourceView for FluentBitView {
    ...
    open spec fn unmarshal(obj: DynamicObjectView) -> Result<FluentBitView, UnmarshalError> {
        if obj.kind != Self::kind() {
            Err(())
        } else if !(FluentBitView::unmarshal_spec(obj.spec) is Ok) {
            Err(())
        } else if !(FluentBitView::unmarshal_status(obj.status) is Ok) {
            Err(())
        } else {
            Ok(FluentBitView {
                metadata: obj.metadata,
                spec: FluentBitView::unmarshal_spec(obj.spec)->Ok_0,
                status: FluentBitView::unmarshal_status(obj.status)->Ok_0,
            })
        }
    }
...
pub struct FluentBitSpecView {
    ...
    pub service_account_annotations: IMap<StringView, StringView>,
    pub service_labels: IMap<StringView, StringView>,
    pub service_selector: Option<IMap<StringView, StringView>>,
    pub service_annotations: IMap<StringView, StringView>,
