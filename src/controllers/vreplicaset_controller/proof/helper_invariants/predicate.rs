...
pub open spec fn every_create_request_is_well_formed(cluster: Cluster, controller_id: int) -> StatePred<ClusterState> {
    |s: ClusterState| {
        forall |msg: Message| #![trigger msg.dst is APIServer, msg.content is APIRequest] {
            let content = msg.content;
...
pub open spec fn filtered_pods_invariant_matrix(controller_id: int, key: ObjectRef, s: ClusterState) -> bool {
    ...
    &&& state.reconcile_step is AfterListPods ==> {
...
pub open spec fn vrs_in_ongoing_reconciles_has_only_one_owner_ref_and_no_deletion_timestamp(
    vrs: VReplicaSetView, controller_id: int,
) -> StatePred<ClusterState> {
