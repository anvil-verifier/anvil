...
pub open spec fn current_state_matches_replicas(vd: VDeploymentView, replicas: int) -> StatePred<ClusterState> {
    |s: ClusterState| {
...
pub open spec fn inductive_current_state_matches_replicas(vd: VDeploymentView, controller_id: int, replicas: int) -> StatePred<ClusterState> {
    |s: ClusterState| {
        &&& current_state_matches_replicas(vd, replicas)(s)
...
pub open spec fn current_state_matches(vd: VDeploymentView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        ...
        exists |k: ObjectRef| {
            ...
            &&& etcd_vrs.metadata.uid is Some
            &&& etcd_vrs.status is Some ==> {
                let status = etcd_vrs.status->0.replicas;
                let replicas = etcd_vrs.spec.replicas.unwrap_or(1);
                if status > vd.spec.replicas.unwrap_or(1) {
                    replicas == status - 1
                } else if status < vd.spec.replicas.unwrap_or(1) {
                    replicas == status + 1
                } else {
                    replicas == status
                }
