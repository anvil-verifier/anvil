...
pub proof fn lemma_filter_old_and_new_vrs_from_resp_objs_implies_etcd_state_is(
    vd: VDeploymentView, cluster: Cluster, controller_id: int, nv_uid_key_replicas_sm: Option<(Uid, ObjectRef, int, bool)>, n: nat, msg: Message, s: ClusterState
)
requires
    cluster.type_is_installed_in_cluster::<VReplicaSetView>(),
    new_vrs_and_old_vrs_of_n_can_be_extracted_from_resp_objs(vd, controller_id, msg, nv_uid_key_replicas_sm, n)(s),
    resp_msg_is_pending_list_resp_in_flight_and_match_req(vd, controller_id, msg)(s),
    s.ongoing_reconciles(controller_id).contains_key(vd.object_ref()),
    cluster_invariants_since_reconciliation(cluster, vd, controller_id)(s),
ensures
    etcd_state_is(vd, controller_id, match nv_uid_key_replicas_sm { Some(t) => Some((t.0, t.1, t.2)), None => None }, n)(s),
{
    ...
    if let Some(new_vrs) = new_vrs_or_none {
        ...
        assert(nv_uid_key_replicas_sm is None);
