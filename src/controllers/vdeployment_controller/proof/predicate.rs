...
pub open spec fn at_vd_step_with_vd(vd: VDeploymentView, controller_id: int, step_pred: spec_fn(ReconcileLocalState) -> bool) -> StatePred<ClusterState> {
    |s: ClusterState| {
        ...
        &&& {
            ...
            // from cr_in_reconciles_has_the_same_spec_uid_name_namespace_and_labels_as_vd, we know:
...
-> StatePred<ClusterState> {
    |s: ClusterState| exists |nv_uid_key: (Uid, ObjectRef)| #![trigger dummy(nv_uid_key)] {
        &&& etcd_state_is(vd, controller_id, Some((nv_uid_key.0, nv_uid_key.1, get_replicas(vd.spec.replicas))), 0)(s)
    }
}
...
pub open spec fn cluster_invariants_since_reconciliation(cluster: Cluster, vd: VDeploymentView, controller_id: int) -> StatePred<ClusterState> {
    and!(
        ...
        helper_invariants::cr_in_reconciles_has_the_same_spec_uid_name_namespace_and_labels_as_vd(vd, controller_id)
    )
}
