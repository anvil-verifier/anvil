        ...
        spec.entails(always(lift_state(cluster_invariants_since_reconciliation(cluster, vd, controller_id)))),
{
    ...
    assert(spec.entails(always(lift_state(Cluster::crash_disabled(controller_id)))));
    assert(spec.entails(always(lift_state(Cluster::req_drop_disabled()))));
    assert(spec.entails(always(lift_state(Cluster::pod_monkey_disabled()))));
    assert(spec.entails(always(lift_state(Cluster::pending_req_of_key_is_unique_with_unique_id(controller_id, vd.object_ref())))));
    assert(spec.entails(always(lift_state(desired_state_is(vd)))));
    assert(spec.entails(always(lift_state(Cluster::every_msg_from_key_is_pending_req_msg_of(controller_id, vd.object_ref())))));
    assert(spec.entails(always(lift_state(helper_invariants::no_other_pending_request_interferes_with_vd_reconcile(vd, controller_id)))));
    assert(spec.entails(always(lift_state(helper_invariants::garbage_collector_does_not_delete_vd_vrs_objects(vd)))));
    assert(spec.entails(always(lift_state(helper_invariants::no_pending_mutation_request_not_from_controller_on_vrs_objects()))));
    assert(spec.entails(always(lift_state(helper_invariants::vd_in_reconciles_has_the_same_spec_uid_name_namespace_and_labels_as_vd(vd, controller_id)))));
    spec_and_invariants_entails_stable_spec_and_invariants(spec, vd, cluster, controller_id);
