       ...
       .leads_to(lift_state(and!(at_vd_step_with_vd(vd, controller_id, at_step![Done]), current_state_matches(vd))))),
{
    ...
    let after_ensure_vrs = |i: (Uid, ObjectRef, nat)| lift_state(and!(
        ...
        local_state_is_coherent_with_etcd(vd, controller_id, Some((i.0, i.1, vd.spec.replicas.unwrap_or(int1!()))), i.2)
    ));
        ...
        assert forall |i: (Option<(Uid, ObjectRef, int)>, nat)| #![trigger after_list_with_etcd_state(msg, i.0, i.1)]
            ...
            if nv_uid_key_replicas is None {
                ...
                let create_vrs_req = lift_state(and!(
                    ...
                    local_state_is_coherent_with_etcd(vd, controller_id, None, n)
                ));
                ...
                let create_vrs_resp = lift_state(and!(
                    at_vd_step_with_vd(vd, controller_id, at_step![AfterCreateNewVRS]),
                    exists_create_resp_msg_with_nv_and_etcd_state_is(vd, controller_id, n),
                    local_state_is_coherent_with_etcd(vd, controller_id, None, n)
                ));
                ...
                if replicas != vd.spec.replicas.unwrap_or(int1!()) {
                    let scale_new_vrs_req = lift_state(and!(
                        ...
                        etcd_state_is(vd, controller_id, nv_uid_key_replicas, n),
                        local_state_is_coherent_with_etcd(vd, controller_id, Some((nv_uid, nv_key, vd.spec.replicas.unwrap_or(int1!()))), n)
                    ));
                    ...
                    assert(spec.entails(after_list_with_etcd_state(msg, nv_uid_key_replicas, n).leads_to(after_ensure_vrs((nv_uid, nv_key, n))))) by {
                        temp_pred_equality(after_ensure_vrs((nv_uid, nv_key, n)), lift_state(and!(
                            ...
                            no_pending_req_in_cluster(vd, controller_id),
                            etcd_state_is(vd, controller_id, nv_uid_key_replicas, n),
                            local_state_is_coherent_with_etcd(vd, controller_id, nv_uid_key_replicas, n)
                        )));
    ...
    assert forall |i: (Uid, ObjectRef, nat)| spec.entails(#[trigger] after_ensure_vrs(i).leads_to(done)) by {
        ...
        if n == 0 {
            ...
            let scale_down_req = lift_state(and!(
                ...
                etcd_state_is(vd, controller_id, nv_uid_key_replicas, n),
                local_state_is_coherent_with_etcd(vd, controller_id, nv_uid_key_replicas, n - nat1!())
            ));
            assert(spec.entails(after_ensure_vrs(i).leads_to(scale_down_req))) by {
                temp_pred_equality(scale_down_req, lift_state(and!(
                    ...
                    pending_scale_old_vrs_req_in_flight(vd, controller_id, nv_uid_key.0),
                    etcd_state_is(vd, controller_id, Some((nv_uid_key.0, nv_uid_key.1, vd.spec.replicas.unwrap_or(int1!()))), n),
                    local_state_is_coherent_with_etcd(vd, controller_id, Some((nv_uid_key.0, nv_uid_key.1, vd.spec.replicas.unwrap_or(int1!()))), n - nat1!())
                )));
            ...
            assert forall |n: nat| #![trigger scale_down_resp(n)] n > 0 implies spec.entails(scale_down_resp(n).leads_to(scale_down_resp((n - nat1!()) as nat))) by {
                temp_pred_equality(scale_down_resp(n), lift_state(and!(
                    ...
                    exists_resp_msg_is_ok_scale_old_vrs_resp_in_flight(vd, controller_id),
                    etcd_state_is(vd, controller_id, Some((nv_uid_key.0, nv_uid_key.1, vd.spec.replicas.unwrap_or(int1!()))), n),
                    local_state_is_coherent_with_etcd(vd, controller_id, Some((nv_uid_key.0, nv_uid_key.1, vd.spec.replicas.unwrap_or(int1!()))), n)
                )));
                temp_pred_equality(scale_down_resp((n - nat1!()) as nat), lift_state(and!(
                    at_vd_step_with_vd(vd, controller_id, at_step![AfterScaleDownOldVRS]),
                    exists_resp_msg_is_ok_scale_old_vrs_resp_in_flight(vd, controller_id),
            ...
            let scale_down_resp_msg_zero = |msg: Message| lift_state(and!(
                ...
                local_state_is_coherent_with_etcd(vd, controller_id, nv_uid_key_replicas, nat0!())
            ));
            ...
            assert forall |msg: Message| spec.entails(#[trigger] scale_down_resp_msg_zero(msg).leads_to(done)) by {
                temp_pred_equality(scale_down_resp_msg_zero(msg), lift_state(and!(
                    ...
                    local_state_is_coherent_with_etcd(vd, controller_id, Some((nv_uid_key.0, nv_uid_key.1, vd.spec.replicas.unwrap_or(int1!()))), nat0!())
                )));
...
ensures
    local_state_is_coherent_with_etcd(vd, controller_id, nv_uid_key_replicas, n)(s_prime),
    ...
    (nv_uid_key_replicas is Some && (nv_uid_key_replicas->0).2 != vd.spec.replicas.unwrap_or(int1!()) ==> {
        ...
        &&& local_state_is_coherent_with_etcd(vd, controller_id, Some(((nv_uid_key_replicas->0).0, (nv_uid_key_replicas->0).1, vd.spec.replicas.unwrap_or(int1!()))), n)(s_prime)
    ...
    (nv_uid_key_replicas is None ==> {
        ...
        &&& local_state_is_coherent_with_etcd(vd, controller_id, None, n)(s_prime)
        ...
        match step {
            Step::APIServerStep(input) => {
                ...
                if msg == req_msg {
                    ...
                    assert(local_state_is_coherent_with_etcd(vd, controller_id, Some((nv_uid_key.0), (nv_uid_key.1), 0), n)(s_prime)) by {
...
ensures
    spec.entails(lift_state(and!(
        ...
        .leads_to(lift_state(and!(
            ...
            etcd_state_is(vd, controller_id, Some(nv_uid_key_replicas), n),
            local_state_is_coherent_with_etcd(vd, controller_id, Some((nv_uid_key_replicas.0, nv_uid_key_replicas.1, vd.spec.replicas.unwrap_or(int1!()))), n)
        )))),
{
    let pre = and!(
        at_vd_step_with_vd(vd, controller_id, at_step![AfterListVRS]),
        ...
        match step {
            Step::APIServerStep(input) => {
                if input->0 == req_msg {
                    ...
                    assert(local_state_is_coherent_with_etcd(vd, controller_id, Some((nv_uid_key.0, nv_uid_key.1, vd.spec.replicas.unwrap_or(int1!()))), (n - nat1!()) as nat)(s_prime));
                }
            },
            _ => {}
        }
