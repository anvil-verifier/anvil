...
pub proof fn lemma_from_list_resp_with_nv_to_next_state(
    s: ClusterState, s_prime: ClusterState, vd: VDeploymentView, cluster: Cluster, controller_id: int, resp_msg: Message, nv_uid_key_replicas_status: Option<(Uid, ObjectRef, int, Option<int>)>
)
requires
    cluster.type_is_installed_in_cluster::<VDeploymentView>(),
    cluster.controller_models.contains_pair(controller_id, vd_controller_model()),
...
ensures
    local_state_is(vd, controller_id, Some(((nv_uid_key_replicas_status->0).0, (nv_uid_key_replicas_status->0).1, (nv_uid_key_replicas_status->0).2)), 0)(s_prime),
    etcd_state_is(vd, controller_id, Some(((nv_uid_key_replicas_status->0).0, (nv_uid_key_replicas_status->0).1, (nv_uid_key_replicas_status->0).2)), 0)(s) ==> local_state_is_valid_and_coherent_with_etcd(vd, controller_id)(s_prime),
    ...
    (nv_uid_key_replicas_status is Some && (nv_uid_key_replicas_status->0).2 == vd.spec.replicas.unwrap_or(int1!()) ==> {
    ...
    (nv_uid_key_replicas_status is Some && (nv_uid_key_replicas_status->0).2 != vd.spec.replicas.unwrap_or(int1!()) ==> {
        ...
        &&& local_state_is(vd, controller_id, Some(((nv_uid_key_replicas_status->0).0, (nv_uid_key_replicas_status->0).1, vd.spec.replicas.unwrap_or(int1!()))), 0)(s_prime)
        &&& pending_scale_new_vrs_req_in_flight(vd, controller_id, ((nv_uid_key_replicas_status->0).0, (nv_uid_key_replicas_status->0).1))(s_prime)
    }),
    (nv_uid_key_replicas_status is None ==> {
        &&& at_vd_step_with_vd(vd, controller_id, at_step![AfterCreateNewVRS])(s_prime)
        &&& local_state_is(vd, controller_id, None, 0)(s_prime)
    ...
    }),
{
    ...
    let new_vrs_uid = if nv_uid_key_replicas_status is Some { Some((nv_uid_key_replicas_status->0).0) } else { None };
