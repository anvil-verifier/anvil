...
pub fn test_set_name() {
    ...
    assert_eq!(
        "name".to_string(),
        config_map_volume_source.into_kube().name.unwrap()
    );
}
...
pub fn test_clone() {
    ...
    assert_eq!(
        config_map_volume_source.into_kube(),
        config_map_volume_source_clone.into_kube()
    );
}
...
pub fn test_kube() {
    ...
    let config_map_volume_source =
        ConfigMapVolumeSource::from_kube(kube_config_map_volume_source.clone());
