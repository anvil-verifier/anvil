...
pub fn test_set_requests() {
    ...
    let quantity_request: BTreeMap<std::string::String, Quantity> = requests
        .into_rust_map()
        .into_iter()
    ...
    assert_eq!(
        quantity_request,
        resource_requirements.into_kube().requests.unwrap()
    );
}
...
pub fn test_set_limits() {
    ...
    let quantity_limits: BTreeMap<std::string::String, Quantity> = limits
        .into_rust_map()
        .into_iter()
        .map(|(k, v)| (k, Quantity(v)))
        .collect();
...
pub fn test_clone() {
    ...
    assert_eq!(
        resource_requirements.into_kube(),
        resource_requirements_clone.into_kube()
    );
}
