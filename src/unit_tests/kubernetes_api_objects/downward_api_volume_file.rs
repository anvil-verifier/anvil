...
pub fn test_default() {
    ...
    assert_eq!(
        downward_api_volume_file.into_kube(),
        deps_hack::k8s_openapi::api::core::v1::DownwardAPIVolumeFile::default()
    );
}
...
pub fn test_set_path() {
    ...
    assert_eq!(
        "path".to_string(),
        downward_api_volume_file.into_kube().path
    );
}

#[test]
pub fn test_kube() {
    let kube_downward_api_volume_file =
    ...
    let downward_api_volume_file =
        DownwardAPIVolumeFile::from_kube(kube_downward_api_volume_file.clone());
