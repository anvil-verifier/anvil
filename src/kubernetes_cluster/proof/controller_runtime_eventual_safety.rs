...
pub open spec fn the_object_in_schedule_has_spec_as<K: ResourceView, R: Reconciler<K>>(cr: K) -> StatePred<State<K, R>> {
    |s: State<K, R>| s.reconcile_scheduled_for(cr.object_ref()) ==> s.reconcile_scheduled_obj_of(cr.object_ref()).spec() == cr.spec()
}
...
pub proof fn lemma_true_leads_to_always_the_object_in_schedule_has_spec_as<K: ResourceView, R: Reconciler<K>>(
    spec: TempPred<State<K, R>>, cr: K
)
    requires
...
pub open spec fn the_object_in_reconcile_has_spec_as<K: ResourceView, R: Reconciler<K>>(cr: K) -> StatePred<State<K, R>> {
    |s: State<K, R>| s.reconcile_state_contains(cr.object_ref()) ==> s.triggering_cr_of(cr.object_ref()).spec() == cr.spec()
}

// This lemma says that under the spec where []desired_state_is(cr), it will eventually reach a state where any object
// in reconcile for cr.object_ref() has the same spec as cr.spec.
pub proof fn lemma_true_leads_to_always_the_object_in_reconcile_has_spec_as<K: ResourceView, R: Reconciler<K>>(
    spec: TempPred<State<K, R>>, cr: K
)
    requires
        K::kind().is_CustomResourceKind(),
        ...
        spec.entails(tla_forall(|i| schedule_controller_reconcile().weak_fairness(i))),
        spec.entails(always(lift_state(desired_state_is(cr)))),
        spec.entails(true_pred().leads_to(lift_state(|s: State<K, R>| !s.reconcile_state_contains(cr.object_ref())))),
        spec.entails(true_pred().leads_to(always(lift_state(the_object_in_schedule_has_spec_as(cr))))),
    ensures
        spec.entails(true_pred().leads_to(always(lift_state(the_object_in_reconcile_has_spec_as(cr))))),
{
    ...
    let stable_spec = always(lift_action(next::<K, R>()))
        .and(tla_forall(|i| schedule_controller_reconcile().weak_fairness(i)))
        .and(tla_forall(|i| controller_next::<K, R>().weak_fairness(i)))
        .and(always(lift_state(desired_state_is(cr))))
        .and(true_pred().leads_to(lift_state(|s: State<K, R>| !s.reconcile_state_contains(cr.object_ref()))))
        ...
        ),
        {
            ...
            assert_by(
                ...
                ),
                {
                    ...
                    K::object_ref_is_well_formed();
                    controller_runtime_liveness::lemma_pre_leads_to_post_by_controller::<K, R>(
                        stable_spec_with_assumption, input, stronger_next,
                        run_scheduled_reconcile::<K, R>(), pre, post
                    );
                }
            );
    ...
    assert_by(
        ...
        {
            always_p_is_stable(lift_action(next::<K, R>()));
            tla_forall_action_weak_fairness_is_stable(schedule_controller_reconcile::<K, R>());
            tla_forall_action_weak_fairness_is_stable(controller_next::<K, R>());
            always_p_is_stable(lift_state(desired_state_is::<K, R>(cr)));
            p_leads_to_q_is_stable(true_pred(), lift_state(|s: State<K, R>| !s.reconcile_state_contains(cr.object_ref())));
            p_leads_to_q_is_stable(true_pred(), always(lift_state(the_object_in_schedule_has_spec_as::<K, R>(cr))));
            ...
            stable_and_n!(
                ...
                always(lift_state(desired_state_is::<K, R>(cr))),
                true_pred().leads_to(lift_state(|s: State<K, R>| !s.reconcile_state_contains(cr.object_ref()))),
    ...
    temp_pred_equality(true_pred().and(always(lift_state(the_object_in_schedule_has_spec_as::<K, R>(cr)))), always(lift_state(the_object_in_schedule_has_spec_as(cr))));
    ...
    entails_and_n!(
        ...
        tla_forall(|i| controller_next::<K, R>().weak_fairness(i)),
