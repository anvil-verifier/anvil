...
pub open spec fn the_object_in_schedule_has_spec_as<K: ResourceView, E: ExternalAPI, R: Reconciler<K, E>>(cr: K) -> StatePred<State<K, E, R>> {
    |s: State<K, E, R>| s.reconcile_scheduled_for(cr.object_ref()) ==> s.reconcile_scheduled_obj_of(cr.object_ref()).spec() == cr.spec()
}
...
pub proof fn lemma_true_leads_to_always_the_object_in_schedule_has_spec_as<K: ResourceView, E: ExternalAPI, R: Reconciler<K, E>>(
    spec: TempPred<State<K, E, R>>, cr: K
)
    requires
...
pub open spec fn the_object_in_reconcile_has_spec_as<K: ResourceView, E: ExternalAPI, R: Reconciler<K, E>>(cr: K) -> StatePred<State<K, E, R>> {
    |s: State<K, E, R>| s.reconcile_state_contains(cr.object_ref()) ==> s.triggering_cr_of(cr.object_ref()).spec() == cr.spec()
}

// This lemma says that under the spec where []desired_state_is(cr), it will eventually reach a state where any object
// in reconcile for cr.object_ref() has the same spec as cr.spec.
pub proof fn lemma_true_leads_to_always_the_object_in_reconcile_has_spec_as<K: ResourceView, E: ExternalAPI, R: Reconciler<K, E>>(
    spec: TempPred<State<K, E, R>>, cr: K
)
    requires
        K::kind().is_CustomResourceKind(),
        ...
        spec.entails(tla_forall(|i| schedule_controller_reconcile().weak_fairness(i))),
        spec.entails(always(lift_state(desired_state_is(cr)))),
        spec.entails(true_pred().leads_to(lift_state(|s: State<K, E, R>| !s.reconcile_state_contains(cr.object_ref())))),
        spec.entails(true_pred().leads_to(always(lift_state(the_object_in_schedule_has_spec_as(cr))))),
    ensures
        spec.entails(true_pred().leads_to(always(lift_state(the_object_in_reconcile_has_spec_as(cr))))),
{
    ...
    let stable_spec = always(lift_action(next::<K, E, R>()))
        ...
        .and(true_pred().leads_to(lift_state(|s: State<K, E, R>| !s.reconcile_state_contains(cr.object_ref()))))
    ...
    assert_by(
        valid(stable(stable_spec)),
        {
            ...
            tla_forall_action_weak_fairness_is_stable(schedule_controller_reconcile::<K, E, R>());
            tla_forall_action_weak_fairness_is_stable(controller_next::<K, E, R>());
            always_p_is_stable(lift_state(desired_state_is::<K, E, R>(cr)));
            p_leads_to_q_is_stable(true_pred(), lift_state(|s: State<K, E, R>| !s.reconcile_state_contains(cr.object_ref())));
            p_leads_to_q_is_stable(true_pred(), always(lift_state(the_object_in_schedule_has_spec_as::<K, E, R>(cr))));
            ...
            stable_and_n!(
                ...
                always(lift_state(desired_state_is::<K, E, R>(cr))),
                true_pred().leads_to(lift_state(|s: State<K, E, R>| !s.reconcile_state_contains(cr.object_ref()))),
    ...
    temp_pred_equality(true_pred().and(always(lift_state(the_object_in_schedule_has_spec_as::<K, E, R>(cr)))), always(lift_state(the_object_in_schedule_has_spec_as(cr))));
