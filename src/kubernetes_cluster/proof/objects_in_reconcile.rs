...
pub open spec fn each_scheduled_object_is_well_formed<T: CustomResourceView>(self, controller_id: int) -> StatePred<ClusterState> {
    |s: ClusterState| {
        forall |key: ObjectRef| {
            &&& #[trigger] s.scheduled_reconciles(controller_id).contains_key(key)
            &&& key.kind == T::kind()
...
pub proof fn lemma_always_each_scheduled_object_is_well_formed<T: CustomResourceView>(self, spec: TempPred<ClusterState>, controller_id: int)
    requires
        spec.entails(lift_state(self.init())),
        spec.entails(always(lift_action(self.next()))),
        self.controller_models.contains_key(controller_id),
        self.type_is_installed_in_cluster::<T>(),
    ensures spec.entails(always(lift_state(self.each_scheduled_object_is_well_formed::<T>(controller_id)))),
{
    let invariant = self.each_scheduled_object_is_well_formed::<T>(controller_id);
    ...
    assert forall |s, s_prime| invariant(s) && #[trigger] stronger_next(s, s_prime) implies invariant(s_prime) by {
        assert forall |key: ObjectRef| #[trigger] s_prime.scheduled_reconciles(controller_id).contains_key(key) && key.kind == T::kind()
        implies {
            let cr = s_prime.scheduled_reconciles(controller_id)[key];
            &&& unmarshallable_object(cr, self.installed_types)
            &&& valid_object(cr, self.installed_types)
        } by {
            let step = choose |step| self.next_step(s, s_prime, step);
...
pub proof fn lemma_always_each_object_in_reconcile_is_well_formed<T: CustomResourceView>(self, spec: TempPred<ClusterState>, controller_id: int)
    requires
        spec.entails(lift_state(self.init())),
        spec.entails(always(lift_action(self.next()))),
        self.type_is_installed_in_cluster::<T>(),
        self.controller_models.contains_key(controller_id),
    ensures spec.entails(always(lift_state(self.each_object_in_reconcile_is_well_formed::<T>(controller_id)))),
{
    let invariant = self.each_object_in_reconcile_is_well_formed::<T>(controller_id);

    self.lemma_always_each_scheduled_object_is_well_formed::<T>(spec, controller_id);
    self.lemma_always_there_is_the_controller_state(spec, controller_id);

    let stronger_next = |s, s_prime| {
        &&& self.next()(s, s_prime)
        &&& self.each_scheduled_object_is_well_formed::<T>(controller_id)(s)
        &&& Self::there_is_the_controller_state(controller_id)(s)
    };

    combine_spec_entails_always_n!(
        spec, lift_action(stronger_next), lift_action(self.next()),
        lift_state(self.each_scheduled_object_is_well_formed::<T>(controller_id)),
        lift_state(Self::there_is_the_controller_state(controller_id))
    );

    assert forall |s, s_prime| invariant(s) && #[trigger] stronger_next(s, s_prime) implies invariant(s_prime) by {
        assert forall |key: ObjectRef| #[trigger] s_prime.ongoing_reconciles(controller_id).contains_key(key) && key.kind == T::kind()
        implies {
            let cr = s_prime.ongoing_reconciles(controller_id)[key].triggering_cr;
            &&& unmarshallable_object(cr, self.installed_types)
            &&& valid_object(cr, self.installed_types)
        } by {
            if s.ongoing_reconciles(controller_id).contains_key(key) {
