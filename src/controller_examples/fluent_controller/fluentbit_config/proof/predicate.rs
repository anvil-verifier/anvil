...
pub open spec fn at_after_create_resource_step_and_exists_ok_resp_in_flight(
    ...
    |s: FBCCluster| {
        ...
        &&& exists |resp_msg| {
            ...
            &&& state_after_create_or_update(sub_resource, resp_msg.content.get_create_response().res.get_Ok_0(), s.ongoing_reconciles()[fbc.object_ref()].local_state).is_Ok()
        }
    }
}
...
pub open spec fn at_after_update_resource_step_and_exists_ok_resp_in_flight(
    ...
    |s: FBCCluster| {
        ...
        &&& exists |resp_msg| {
            ...
            &&& state_after_create_or_update(sub_resource, resp_msg.content.get_update_response().res.get_Ok_0(), s.ongoing_reconciles()[fbc.object_ref()].local_state).is_Ok()
        }
    }
}
