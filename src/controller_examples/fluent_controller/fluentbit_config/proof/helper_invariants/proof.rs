...
pub proof fn lemma_always_the_object_in_reconcile_satisfies_state_validation(spec: TempPred<FBCCluster>)
    requires
        spec.entails(lift_state(FBCCluster::init())),
        spec.entails(always(lift_action(FBCCluster::next()))),
    ensures
...
pub proof fn lemma_eventually_always_every_resource_update_request_implies_at_after_update_resource_step_forall(
    ...
    requires
        ...
        spec.entails(always(lift_state(FBCCluster::each_object_in_etcd_is_well_formed()))),
        spec.entails(always(lift_state(FBCCluster::every_in_flight_req_is_unique()))),
        ...
        ),
{
    ...
    let stronger_next = |s: FBCCluster, s_prime: FBCCluster| {
        ...
        &&& FBCCluster::every_in_flight_req_is_unique()(s)
...
pub proof fn lemma_resource_create_or_update_request_msg_implies_key_in_reconcile_equals(
    ...
    ensures
        ...
        ==> step.is_ControllerStep() && step.get_ControllerStep_0().1.get_Some_0() == fbc.object_ref()
            ...
            && FBCCluster::pending_k8s_api_req_msg_is(s_prime, fbc.object_ref(), msg),
            ...
            && FBCCluster::pending_k8s_api_req_msg_is(s_prime, fbc.object_ref(), msg),
{
    ...
    if resource_create_request_msg(get_request(sub_resource, fbc).key)(msg) || resource_update_request_msg(get_request(sub_resource, fbc).key)(msg) {
        ...
        assert_by(
            cr_key == fbc.object_ref() && local_step.get_AfterKRequestStep_1() == sub_resource && FBCCluster::pending_k8s_api_req_msg_is(s_prime, cr_key, msg),
            {
        ...
        spec.entails(true_pred().leads_to(always(lift_state(resource_object_only_has_owner_reference_pointing_to_current_cr(sub_resource, fbc))))),
{
    ...
    always_weaken(spec, object_in_every_resource_update_request_only_has_owner_references_pointing_to_current_cr(sub_resource, fbc), FBCCluster::every_update_msg_sets_owner_references_as(key, eventual_owner_ref));
    always_weaken(spec, every_resource_create_request_implies_at_after_create_resource_step(sub_resource, fbc), FBCCluster::every_create_msg_sets_owner_references_as(key, eventual_owner_ref));
    always_weaken(spec, resource_object_has_no_finalizers_or_timestamp_and_only_has_controller_owner_ref(sub_resource, fbc), FBCCluster::object_has_no_finalizers(key));
