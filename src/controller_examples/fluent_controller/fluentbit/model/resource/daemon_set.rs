    ...
    PodSpecView {
        ...
        volumes: Some({
            ...
            if fb.spec.volumes.is_Some() {
                fb.spec.volumes.get_Some_0().push(config_volume) + make_log_volumes(fb)
            } else {
                seq![config_volume] + make_log_volumes(fb)
            }
        }),
        containers: seq![
            ContainerView {
...
pub open spec fn make_log_volumes(fb: FluentBitView) -> Seq<VolumeView> {
    ...
    // seq![varlibcontainers_volume, varlogs_volume]
}
