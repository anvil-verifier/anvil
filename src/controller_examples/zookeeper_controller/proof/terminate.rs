...
use crate::kubernetes_cluster::{
    proof::{
        ...
        controller_runtime_liveness::{
            lemma_from_in_flight_resp_matches_pending_req_at_some_state_to_next_state,
            lemma_from_pending_req_in_flight_at_some_state_to_next_state,
            lemma_from_some_state_to_next_state_to_reconcile_idle,
...
pub proof fn lemma_from_init_step_to_next_step(spec: TempPred<ClusterState>, zk: ZookeeperClusterView)
    ...
    ensures
        spec.entails(
            lift_state(reconciler_reconcile_init::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>(zk.object_ref()))
                .leads_to(lift_state(at_reconcile_state(zk.object_ref(), zookeeper_reconcile_state(ZookeeperReconcileStep::AfterCreateHeadlessService))))
        ),
{
    temp_pred_equality::<ClusterState>(
    ...
    lemma_pre_leads_to_post_by_controller::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>(
        ...
        at_reconcile_state(zk.object_ref(), zookeeper_reconcile_state(ZookeeperReconcileStep::AfterCreateHeadlessService))
    );
    leads_to_trans_temp(
        spec,
        lift_state(reconciler_reconcile_init::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>(zk.object_ref())),
