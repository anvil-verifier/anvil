...
pub proof fn lemma_from_some_step_to_next_step_to_reconcile_idle(
    ...
    requires
        ...
        spec.entails(always(lift_state(controller_runtime_safety::each_resp_matches_at_most_one_pending_req(zk.object_ref())))),
        spec.entails(always(lift_state(controller_runtime_safety::each_resp_if_matches_pending_req_then_no_other_resp_matches(zk.object_ref())))),
        spec.entails(always(lift_state(pending_req_in_flight_or_resp_in_flight_at_step(zk.object_ref(), step)))),
        ...
        ),
{
    let at_some_step_and_pending_req_in_flight_or_resp_in_flight = |s: ClusterState| {
        at_zookeeper_step(zk.object_ref(), step)(s)
        && s.reconcile_state_of(zk.object_ref()).pending_req_msg.is_Some()
        && is_controller_request(s.pending_req_of(zk.object_ref()))
        && (s.message_in_flight(s.pending_req_of(zk.object_ref()))
        || exists |resp_msg: Message| {
    ...
    temp_pred_equality::<ClusterState>(lift_state(pending_req_in_flight_or_resp_in_flight_at_step(zk.object_ref(), step)), lift_state(at_zookeeper_step(zk.object_ref(), step)).implies(lift_state(at_some_step_and_pending_req_in_flight_or_resp_in_flight)));
    implies_to_leads_to::<ClusterState>(spec, lift_state(at_zookeeper_step(zk.object_ref(), step)), lift_state(at_some_step_and_pending_req_in_flight_or_resp_in_flight));

    let req_in_flight = pending_req_in_flight_at_step(zk.object_ref(), step);
    let resp_in_flight = resp_in_flight_matches_pending_req_at_step(zk.object_ref(), step);

    let done_step = reconciler_reconcile_done::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>(zk.object_ref());
    lemma_from_step_a_and_resp_matches_pending_req_in_flight_to_step_b(spec, zk, step, next_step);
    lemma_from_pending_req_in_flight_at_step_a_to_step_b(spec, zk, step, next_step);
    or_leads_to_combine(spec, req_in_flight, resp_in_flight, at_zookeeper_step(zk.object_ref(), next_step));
...
pub proof fn lemma_from_init_step_to_next_step(spec: TempPred<ClusterState>, zk: ZookeeperClusterView)
    requires
        zk.well_formed(),
        spec.entails(always(lift_action(next::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>()))),
        spec.entails(tla_forall(|i| kubernetes_api_next().weak_fairness(i))),
    ...
    ensures
        spec.entails(
            lift_state(reconciler_reconcile_init::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>(zk.object_ref()))
                .leads_to(lift_state(at_zookeeper_step(zk.object_ref(), ZookeeperReconcileStep::AfterCreateHeadlessService)))
        ),
{
    temp_pred_equality::<ClusterState>(
...
proof fn lemma_from_at_after_get_stateful_set_step_and_pending_req_in_flight_to_reconcile_idle(spec: TempPred<ClusterState>, zk: ZookeeperClusterView)
    ...
    ensures
        spec.entails(
            lift_state(pending_req_in_flight_at_step(zk.object_ref(), ZookeeperReconcileStep::AfterGetStatefulSet))
                .leads_to(lift_state(|s: ClusterState| { !s.reconcile_state_contains(zk.object_ref()) }))
        ),
{
...
proof fn lemma_from_pending_req_in_flight_at_step_a_to_step_b(
    ...
    ensures
        spec.entails(
            lift_state(pending_req_in_flight_at_step(zk.object_ref(), step_a))
