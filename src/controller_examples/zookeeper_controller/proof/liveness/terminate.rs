...
pub proof fn reconcile_eventually_terminates(spec: TempPred<ZKCluster>, zookeeper: ZookeeperClusterView)
    ...
    ensures
        spec.entails(
            true_pred().leads_to(lift_state(|s: ZKCluster| !s.ongoing_reconciles().contains_key(zookeeper.object_ref())))
        ),
{
...
pub open spec fn at_step1_or_step2_or_step3_closure(step1: ZookeeperReconcileStep, step2: ZookeeperReconcileStep, step3: ZookeeperReconcileStep) -> FnSpec(ZookeeperReconcileState) -> bool {
