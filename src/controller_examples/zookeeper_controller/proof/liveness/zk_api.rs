        ...
        ),
{
    ...
    let stronger_next = |s, s_prime: ZKCluster| {
        ...
        &&& helper_invariants::no_update_status_request_msg_in_flight_of_except_stateful_set(SubResource::StatefulSet, zookeeper)(s)
    };
    combine_spec_entails_always_n!(
        spec, lift_action(stronger_next),
        lift_action(ZKCluster::next()),
        ...
        lift_state(helper_invariants::no_update_status_request_msg_in_flight_of_except_stateful_set(SubResource::StatefulSet, zookeeper))
    );
    ...
    ensures
        spec.entails(
            lift_state(
                |s: ZKCluster| {
                    &&& s.resources().contains_key(get_request(SubResource::StatefulSet, zookeeper).key)
                    &&& s.external_state().data.contains_key(addr_from_exists_request(s, zookeeper))
        ...
        match step {
            Step::ExternalAPIStep(input) => {
                if input.get_Some_0() == req_msg {
                    ...
                    let addr = addr_from_exists_request(s_prime, zookeeper);
                    assert({
                        &&& s_prime.in_flight().contains(resp_msg)
                        &&& Message::resp_msg_matches_req_msg(resp_msg, req_msg)
