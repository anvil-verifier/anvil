...
pub proof fn lemma_from_after_get_zk_step_to_after_get_stateful_set_step(
    ...
    requires
        ...
        spec.entails(always(lift_state(helper_invariants::no_update_status_request_msg_in_flight_of_except_stateful_set(SubResource::StatefulSet, zookeeper)))),
        ...
        ),
{
    ...
    assert_by(spec.entails(pre.leads_to(post)), {
        ...
        assert forall |resp_msg| spec.entails(#[trigger] pre_and_resp_in_flight(resp_msg).leads_to(post)) by {
            lemma_from_after_get_zk_step_to_after_create_resource_step(spec, zookeeper, resp_msg);
        }
...
proof fn lemma_from_after_get_zk_step_to_after_create_resource_step(
...
//         let resp = ZKCluster::handle_create_request(pending_msg, s.kubernetes_api_state).1;
...
//     assert forall |s, s_prime: ZKCluster| pre(s) && #[trigger] stronger_next(s, s_prime) && ZKCluster::kubernetes_api_next().forward(input)(s, s_prime) implies post(s_prime) by {
//         let pending_msg = s.ongoing_reconciles()[zookeeper.object_ref()].pending_req_msg.get_Some_0();
//         let resp = ZKCluster::handle_update_request(pending_msg, s.kubernetes_api_state).1;
//         assert(s_prime.in_flight().contains(resp));
//         match sub_resource {
//             SubResource::HeadlessService => ServiceView::marshal_preserves_integrity(),
//             SubResource::ClientService => ServiceView::marshal_preserves_integrity(),
...
// proof fn lemma_from_after_get_zk_step_to_after_update_resource_step(
