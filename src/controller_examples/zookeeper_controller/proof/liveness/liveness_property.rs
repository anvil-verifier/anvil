...
use crate::zookeeper_controller::{
    common::*,
    proof::{common::*, liveness::help_invariants, liveness::terminate},
...
spec fn invariants(zk: ZookeeperClusterView) -> TempPred<ClusterState> {
    ...
    .and(always(lift_state(help_invariants::pending_msg_at_after_create_stateful_set_step_is_create_sts_req(zk.object_ref()))))
    .and(always(lift_state(help_invariants::pending_msg_at_after_update_stateful_set_step_is_update_sts_req(zk.object_ref()))))
        ...
        valid(stable(invariants(zk))),
{
    stable_and_always_n!(
        ...
        lift_state(help_invariants::pending_msg_at_after_create_stateful_set_step_is_create_sts_req(zk.object_ref())),
        lift_state(help_invariants::pending_msg_at_after_update_stateful_set_step_is_update_sts_req(zk.object_ref())),
...
spec fn invariants_since_rest_id(zk: ZookeeperClusterView, rest_id: RestId) -> TempPred<ClusterState> {
    ...
    .and(always(lift_state(help_invariants::at_most_one_create_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id))))
    .and(always(lift_state(help_invariants::at_most_one_update_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id))))
    .and(always(lift_state(help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id))))
    .and(always(lift_state(help_invariants::every_update_sts_req_since_rest_id_does_the_same(zk, rest_id))))
}
        ...
        valid(stable(invariants_since_rest_id(zk, rest_id))),
{
    stable_and_always_n!(
        ...
        lift_state(help_invariants::at_most_one_create_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)),
        lift_state(help_invariants::at_most_one_update_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)),
        lift_state(help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)),
        lift_state(help_invariants::every_update_sts_req_since_rest_id_does_the_same(zk, rest_id))
    );
}
        ...
        cluster_spec().entails(liveness(zk)),
{
    ...
    assert_by(
        ...
        ),
        {
            ...
            help_invariants::lemma_always_pending_msg_at_after_create_stateful_set_step_is_create_sts_req(spec, zk.object_ref());
            help_invariants::lemma_always_pending_msg_at_after_update_stateful_set_step_is_update_sts_req(spec, zk.object_ref());
            ...
            entails_and_n!(
                ...
                always(lift_state(help_invariants::pending_msg_at_after_create_stateful_set_step_is_create_sts_req(zk.object_ref()))),
                always(lift_state(help_invariants::pending_msg_at_after_update_stateful_set_step_is_update_sts_req(zk.object_ref()))),
        ...
        ),
{
    ...
    assert_by(
        spec.entails(invariants_since_rest_id(zk, rest_id)),
        {
            ...
            eliminate_always(spec, lift_state(help_invariants::pending_msg_at_after_create_stateful_set_step_is_create_sts_req(zk.object_ref())));
            eliminate_always(spec, lift_state(help_invariants::pending_msg_at_after_update_stateful_set_step_is_update_sts_req(zk.object_ref())));
            ...
            help_invariants::lemma_always_at_most_one_create_sts_req_since_rest_id_is_in_flight(spec, zk.object_ref(), rest_id);
            help_invariants::lemma_always_at_most_one_update_sts_req_since_rest_id_is_in_flight(spec, zk.object_ref(), rest_id);
            help_invariants::lemma_always_no_delete_sts_req_since_rest_id_is_in_flight(spec, zk.object_ref(), rest_id);
            help_invariants::lemma_always_every_update_sts_req_since_rest_id_does_the_same(spec, zk, rest_id);

            entails_and_n!(
                spec,
                always(lift_state(rest_id_counter_is_no_smaller_than(rest_id))),
                always(lift_state(help_invariants::at_most_one_create_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id))),
                always(lift_state(help_invariants::at_most_one_update_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id))),
                always(lift_state(help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id))),
                always(lift_state(help_invariants::every_update_sts_req_since_rest_id_does_the_same(zk, rest_id)))
            );
        }
    );
...
proof fn lemma_receives_ok_resp_at_after_get_stateful_set_step_with_zk(
    ...
    requires
        ...
        spec.entails(always(lift_state(help_invariants::at_most_one_update_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)))),
        spec.entails(always(lift_state(help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)))),
        zk.well_formed(),
    ensures
        spec.entails(
            lift_state(
        ...
        ),
{
    ...
    let stronger_next = |s, s_prime: ClusterState| {
        ...
        &&& help_invariants::at_most_one_update_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)(s)
        &&& help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)(s)
    };
    entails_always_and_n!(
        spec,
        lift_action(next::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>()),
    ...
    temp_pred_equality(
        ...
        .and(lift_state(help_invariants::at_most_one_update_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)))
        .and(lift_state(help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)))
    );
...
proof fn lemma_receives_not_found_resp_at_after_get_stateful_set_step_with_zk(
    ...
    requires
        ...
        spec.entails(always(lift_state(controller_runtime_safety::every_in_flight_msg_has_unique_id()))),
        spec.entails(always(lift_state(kubernetes_api_liveness::no_req_before_rest_id_is_in_flight(rest_id)))),
        spec.entails(always(lift_state(help_invariants::at_most_one_create_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)))),
        zk.well_formed(),
    ensures
        spec.entails(
            lift_state(
        ...
        ),
{
    ...
    let stronger_next = |s, s_prime: ClusterState| {
        ...
        &&& controller_runtime_safety::every_in_flight_msg_has_unique_id()(s)
        &&& kubernetes_api_liveness::no_req_before_rest_id_is_in_flight(rest_id)(s)
        &&& help_invariants::at_most_one_create_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)(s)
    };
    entails_always_and_n!(
        spec,
        lift_action(next::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>()),
    ...
    temp_pred_equality(
        ...
        .and(lift_state(controller_runtime_safety::every_in_flight_msg_has_unique_id()))
        .and(lift_state(kubernetes_api_liveness::no_req_before_rest_id_is_in_flight(rest_id)))
        .and(lift_state(help_invariants::at_most_one_create_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)))
    );
...
proof fn lemma_stateful_set_is_stable(
    ...
    requires
        ...
        spec.entails(always(lift_state(kubernetes_api_liveness::no_req_before_rest_id_is_in_flight(rest_id)))),
        spec.entails(always(lift_state(help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)))),
        spec.entails(always(lift_state(help_invariants::every_update_sts_req_since_rest_id_does_the_same(zk, rest_id)))),
    ensures
        spec.entails(p.leads_to(always(lift_state(current_state_matches(zk))))),
{
    ...
    let stronger_next = |s, s_prime: ClusterState| {
        ...
        &&& kubernetes_api_liveness::no_req_before_rest_id_is_in_flight(rest_id)(s)
        &&& help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)(s)
        &&& help_invariants::every_update_sts_req_since_rest_id_does_the_same(zk, rest_id)(s)
    };
    entails_always_and_n!(
        spec,
        lift_action(next::<ZookeeperClusterView, ZookeeperReconcileState, ZookeeperReconciler>()),
    ...
    temp_pred_equality(
        ...
        .and(lift_state(kubernetes_api_liveness::no_req_before_rest_id_is_in_flight(rest_id)))
        .and(lift_state(help_invariants::no_delete_sts_req_since_rest_id_is_in_flight(zk.object_ref(), rest_id)))
        .and(lift_state(help_invariants::every_update_sts_req_since_rest_id_does_the_same(zk, rest_id)))
    );
