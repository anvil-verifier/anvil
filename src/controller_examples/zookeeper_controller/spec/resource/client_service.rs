...
impl ResourceBuilder<ZookeeperClusterView, ZookeeperReconcileState> for ClientServiceBuilder {
    ...
    open spec fn update(rabbitmq: ZookeeperClusterView, state: ZookeeperReconcileState, obj: DynamicObjectView) -> Result<DynamicObjectView, ()> {
        ...
        if service.is_Ok() && service.get_Ok_0().spec.is_Some() {
            Ok(update_client_service(rabbitmq, service.get_Ok_0()).marshal())
        } else {
            Err(())
        }
    }
    ...
    open spec fn resource_state_matches(rabbitmq: ZookeeperClusterView, resources: StoredState) -> bool {
        let key = make_client_service_key(rabbitmq);
        let obj = resources[key];
        let made_spec = make_client_service(rabbitmq).spec.get_Some_0();
        ...
        &&& obj.metadata.labels == make_client_service(rabbitmq).metadata.labels
        &&& obj.metadata.annotations == make_client_service(rabbitmq).metadata.annotations
    }
