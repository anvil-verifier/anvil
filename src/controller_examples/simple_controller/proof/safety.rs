        ...
        reconcile_get_cr_done_implies_pending_req_in_flight_or_resp_in_flight(cr)(s_prime),
{
    ...
    if s_prime.ongoing_reconciles().contains_key(cr.object_ref()) && s_prime.ongoing_reconciles()[cr.object_ref()].local_state.reconcile_pc == reconciler::after_get_cr_pc() {
        // Depending on whether reconcile state is at after_get_cr_pc, we need to reason about different transitions
        if s.ongoing_reconciles().contains_key(cr.object_ref()) && s.ongoing_reconciles()[cr.object_ref()].local_state.reconcile_pc == reconciler::after_get_cr_pc() {
            let req_msg = choose |req_msg| #[trigger] is_controller_get_cr_request_msg(req_msg, cr) && s.ongoing_reconciles()[cr.object_ref()].pending_req_msg == Some(req_msg);
            assert(is_controller_get_cr_request_msg(req_msg, cr) && s_prime.ongoing_reconciles()[cr.object_ref()].pending_req_msg == Some(req_msg));
            ...
            if s.in_flight().contains(req_msg) {
                ...
                if s_prime.in_flight().contains(req_msg) {
                    assert(s_prime.in_flight().contains(req_msg)); // providing witness for exists |req_msg| ...
                } else {
                    let (_, resp_msg, _) = handle_get_request(req_msg, s.kubernetes_api_state);
                    assert(s_prime.in_flight().contains(resp_msg) && Message::resp_msg_matches_req_msg(resp_msg, req_msg)); // providing witness for exists |resp_msg| ...
                }
            } else {
                // If req_msg is not in flight at s, then the corresponding resp_msg is in flight at s
                let resp_msg = choose |resp_msg| #[trigger] s.in_flight().contains(resp_msg) && Message::resp_msg_matches_req_msg(resp_msg, req_msg);
            ...
            assert(is_controller_get_cr_request_msg(req_msg, cr)
                && s_prime.ongoing_reconciles()[cr.object_ref()].pending_req_msg == Some(req_msg)
                && s_prime.in_flight().contains(req_msg)
...
pub open spec fn delete_cm_req_msg_not_in_flight(cr: SimpleCRView) -> StatePred<State<SimpleReconcileState>> {
    |s: State<SimpleReconcileState>| {
        !exists |m: Message| {
            &&& #[trigger] s.in_flight().contains(m)
        ...
        sm_spec(simple_reconciler()).entails(always(lift_state(delete_cm_req_msg_not_in_flight(cr)))),
{
    ...
    assert forall |s, s_prime: State<SimpleReconcileState>| invariant(s) && #[trigger] next(simple_reconciler())(s, s_prime) implies invariant(s_prime) by {
        assert(!exists |m: Message| s.in_flight().contains(m)
            && m.dst == HostId::KubernetesAPI
